...
def find_path(marker_pub, grid, start, goal):
    ...
    """
    (start_col, start_row) = get_cell_for_point(grid, start)
    (goal_col, goal_row) = get_cell_for_point(grid, goal)

    if start_col < 0 or start_col >= grid.info.width or start_row < 0 or start_row >= grid.info.height or \
       goal_col  < 0 or goal_col  >= grid.info.width or goal_row  < 0 or goal_row  >= grid.info.height:       
    ...
    visualize_cell(marker_pub, grid, start_col, start_row, ColorRGBA(0, 1.0, 1.0, 1.0))
    visualize_cell(marker_pub, grid, goal_col, goal_row, ColorRGBA(0, 0.7, 0, 1.0))

    if is_occupied(grid, start_col, start_row):
        rospy.logwarn("No path could be found. Starting point %s is in an occupied cell" % format_point(start))
        return []
    
    if is_occupied(grid, goal_col, goal_row):
    ...
    def get_empty_neighbors(col, row):
        ...
        onmap_neighbors = [n for n in neighbors if n[0] >= 0 and n[0] < grid.info.width and n[1] >= 0 and n[1] < grid.info.height]
        return [n for n in onmap_neighbors if not is_occupied(grid, col, row)]
    ...
    while cell in back_pointers:
        ...
        npoint = get_point_for_cell(grid, cell[0], cell[1])
    ...
    if len(poses) is 0:
        ...
        rospy.loginfo("Found path of length %0.2f found from start %s to goal %s! (%d cells)" % (final_path_length, format_point(start), format_point(goal), len(poses)))
    return poses

def format_point(point):
    return "(%0.3f, %0.3f)" % (point.x, point.y)

def publish_path(marker_pub, pose_seq):
    
    # Publish poses (for use by robot)
    ...
    marker = Marker(
            ...
            action=Marker.ADD,
            id=0,
            lifetime=rospy.Duration(VISUALIZATION_TIME), # forever
...
def is_occupied(grid, col, row):
    return grid.data[col + grid.info.width * row] > 50
...
def visualize_cell(marker_pub, grid, col, row, color):
    marker = Marker(
            type=Marker.SPHERE,
            action=Marker.ADD,
            id= (1 + col * grid.info.width + row),
            lifetime=rospy.Duration(secs=VISUALIZATION_TIME),
            scale=Vector3(0.3, 0.3, 0.3),
            header=Header(frame_id='map'),
            pose=Pose(position=get_point_for_cell(grid, col, row)),
            color=color
    )
    marker_pub.publish(marker)
    
def visualize_grid_occupancy(marker_pub, grid):
    row = 0
    rate = rospy.Rate(4)
    ...
    while not rospy.is_shutdown():
        ...
        marker = Marker(
            ...
            action=Marker.ADD,
            id=0,
            lifetime=rospy.Duration(secs=0.25),
            scale=Vector3(0.1, 0.1, 0.1),
            header=Header(frame_id='map'),
            points=points,
            colors=colors
...
if __name__ == "__main__":  
    ...
    rospy.loginfo("Grid Info: " + str(grid.info))

    pose_seq = find_path(marker_pub, grid, start, end)
    publish_path(marker_pub, pose_seq)
    # visualize_grid_occupancy(marker_pub, grid)